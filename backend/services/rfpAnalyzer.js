  ...
  async analyzeRFP(input, source = 'uploaded_rfp.pdf') {
    try {
      ...
      const result = {
        ...
        submissionDeadline: extractedData.submissionDeadline || 'Not available',
        projectDeadline: extractedData.projectDeadline || 'Not available',
        ...
        timelineMilestones: Array.isArray(extractedData.timelineMilestones)
          ? extractedData.timelineMilestones
          : [],
      ...
      const ensureStringArray = (a) => {
        if (!a) return ['Not available']
        if (!Array.isArray(a)) return [String(a)]
        return a.length ? a.map((x) => String(x)) : ['Not available']
      }

      result.keyRequirements = ensureStringArray(result.keyRequirements)
      result.evaluationCriteria = ensureStringArray(result.evaluationCriteria)
      result.deliverables = ensureStringArray(result.deliverables)
      result.additionalInfo = ensureStringArray(result.additionalInfo)
      result.criticalInformation = ensureStringArray(result.criticalInformation)
      result.clarificationQuestions = ensureStringArray(
