...
export const renderSectionContent = (
    ...
    if (typeof content === 'object' && content !== null) {
      ...
      return `
          <div class="title-section">
            ${
              titleData.submittedBy
                ? `<p><strong>Submitted by:</strong> ${titleData.submittedBy}</p>`
                : ''
            }
            ${
              titleData.name
                ? `<p><strong>Name:</strong> ${titleData.name}</p>`
                : ''
            }
            ${
              titleData.email
                ? `<p><strong>Email:</strong> ${titleData.email}</p>`
                : ''
            }
            ${
              titleData.number
                ? `<p><strong>Number:</strong> ${titleData.number}</p>`
                : ''
            }
          </div>
        `
  ...
  // Check if this section contains table data (Budget, Timeline, Schedule, Methodology, Technical Approach, etc.)
  const isTableSection =
    sectionName.toLowerCase().includes('budget') ||
    sectionName.toLowerCase().includes('timeline') ||
    sectionName.toLowerCase().includes('schedule') ||
    sectionName.toLowerCase().includes('milestone') ||
...
// Helper to detect "Title" / cover-page sections (these should NOT show "Select from Library")
export const isTitleSectionName = (sectionName: string): boolean => {
  const title = (sectionName || '').trim().toLowerCase()
  if (!title) return false

  // Common variants seen in RFP templates
  return (
    title === 'title' ||
    title === 'title page' ||
...
export const getContentLibraryType = (
  sectionName: string,
): 'team' | 'references' | 'company' | null => {
  const title = sectionName.toLowerCase()

  // Check for Title section
  if (isTitleSectionName(sectionName)) {
    return 'company'
  }
